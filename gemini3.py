...
Quertion: Author with the most posts
MongoDB Query: collection.find({{
    "method": "aggregate",
    "args": [[{{ "$unwind": "$posts" }}, {{ "$match": {{ "posts.category": "GenAI" }} }}]]
}})
...
if __name__ == "__main__":
    app.run(port=4005, host="0.0.0.0", debug=False)
...
def get_flask_response(x):
    ...
    response = requests.post("http://127.0.0.1:4005/query-rag", json={"query": query})
...
evaluate(
    ...
    # experiment_prefix="gemini-2.0-flash-lite"
    experiment_prefix="gemma-3-27b-it"
)
